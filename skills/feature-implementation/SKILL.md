...
Do not make the user repeat available information. Resolve low-cost implementation details from repository evidence. Ask only when a missing product judgment or authorization would materially change the result.
...
- **Explorer:** Trace a subsystem, inventory tests or callers, or validate an assumption without editing.
...
3. Integrate every required path for the slice. Avoid dormant code, speculative extension points, silent fallbacks, and duplicate sources of truth.
